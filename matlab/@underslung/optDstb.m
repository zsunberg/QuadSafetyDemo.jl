...
if strcmp(dMode, 'max')
  ...
  if any(obj.dims == 2)
    dOpt{2} = (deriv{obj.dims==2}>=0)*obj.dMax(2) + ...
      (deriv{obj.dims==2}<0)*(obj.dMin(2));
  end

  if any(obj.dims == 3)
    dOpt{3} = (deriv{obj.dims==3}>=0)*obj.dMax(3) + ...
      (deriv{obj.dims==3}<0)*(obj.dMin(3));
  end
  

  if any(obj.dims == 4)
    dOpt{4} = (deriv{obj.dims==4}>=0)*obj.dMax(4) + ...
      (deriv{obj.dims==4}<0)*(obj.dMin(4));
  end
  
elseif strcmp(dMode, 'min')
  if any(obj.dims == 1)
    dOpt{1} = (deriv{obj.dims==1}>=0)*(obj.dMin(1)) + ...
      (deriv{obj.dims==1}<0)*obj.dMax(1);
  end
  
  if any(obj.dims == 2)
    dOpt{2} = (deriv{obj.dims==2}>=0)*(obj.dMin(2)) + ...
      (deriv{obj.dims==2}<0)*obj.dMax(2);
  end
  
  if any(obj.dims == 3)
    dOpt{3} = (deriv{obj.dims==3}>=0)*obj.dMin(3) + ...
      (deriv{obj.dims==3}<0)*(obj.dMax(3));
  end
  

  if any(obj.dims == 4)
    dOpt{4} = (deriv{obj.dims==4}>=0)*obj.dMin(4) + ...
